...
if (typeof window.aivContentScriptLoaded === 'undefined') {
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    console.log('[AIV Content] Received message:', request);
    if (request.action === 'toggleValidation') {
      isValidationEnabled = request.enabled;
      
      if (isValidationEnabled) {
        startObserving();
        console.log('[AIV Content] AIV Validation enabled');
      } else {
        stopObserving();
        console.log('[AIV Content] AIV Validation disabled');
      }
      ...
      triggerManualValidation()
        ...
        .catch(error => {
          console.error('[AIV Content] Error in triggerManualValidation:', error);
          sendResponse({ success: false, error: error.message });
        });
      return true; // Keep the message channel open for async responses
    } else if (request.action === 'getConversationPairs') {
      // Send the conversation pairs to the popup
      console.log('[AIV Content] Sending conversation pairs to popup:', conversationPairs);
  ...
  function checkForIncompleteResponses() {
    for (let i = 0; i < conversationPairs.length; i++) {
      ...
      if (pair.response && !isCompleteResponse(pair.response, null)) {
        console.log('[AIV Content] Found incomplete response in pair', i, ':', pair.response);
        ...
        if (moreCompleteResponse && moreCompleteResponse !== pair.response) {
          console.log('[AIV Content] Found more complete response for pair', i, ':', moreCompleteResponse.substring(0, 50) + '...');
          ...
          if (trimmedResponse.length < 10) {
            console.log('[AIV Content] Removing very short incomplete response from pair', i);
  ...
  function captureExistingContent() {
    ...
    console.log('[AIV Content] Capturing existing content');
  ...
  function processMessage(messageElement) {
    console.log('[AIV Content] processMessage called with element:', messageElement);
    console.log('[AIV Content] Element tag name:', messageElement.tagName);
    console.log('[AIV Content] Element class list:', messageElement.classList);
    
    // Only process if validation is enabled
    if (!isValidationEnabled) {
      console.log('[AIV Content] Validation not enabled, skipping message processing');
      return;
    }
    ...
    if (now - lastProcessingTime < 100) { // Reduced to 100ms to catch streaming responses
      console.log('[AIV Content] Too soon since last processing, skipping');
      return;
    }
    ...
    console.log('[AIV Content] Extracted message data:', messageData);
    
    if (messageData && (messageData.role === 'user' || messageData.role === 'assistant')) {
      console.log(`[AIV Content] Processing ${messageData.role} message:`, messageData.content.substring(0, 50) + '...');
      ...
      if (messageData.role === 'assistant') {
        ...
        if (!isCompleteResponse(messageData.content, messageElement)) {
          console.log('[AIV Content] Assistant response appears incomplete, skipping for now');
          return;
        }
        ...
        if (conversationPairs.length > 0) {
          ...
          if (lastPair && !lastPair.response && messageData.content) {
            ...
            console.log('[AIV Content] Found complete response for last pair');
          ...
          if (lastPair && lastPair.response && 
            ...
            console.log('[AIV Content] Updating last pair with more complete response');
            lastPair.response = messageData.content;
            return;
          }
        }
      ...
      if (lastProcessedMessages.has(messageKey)) {
        console.log('[AIV Content] Message already processed, skipping');
        return;
      }
      ...
      console.log(`[AIV Content] Processing message: ${messageData.role} - ${messageData.content.substring(0, 50)}...`);
      
      // Add to conversation pairs
      addToConversationPairs(messageData);
    } else {
      console.log('[AIV Content] Skipping message - invalid data or unsupported role');
      if (!messageData) {
        console.log('[AIV Content] Reason: No message data extracted');
      } else if (messageData.role !== 'user' && messageData.role !== 'assistant') {
        console.log('[AIV Content] Reason: Unsupported role:', messageData.role);
      }
    }
  }
  ...
  function addToConversationPairs(messageData) {
    console.log('[AIV Content] addToConversationPairs called with:', messageData);
    
    // For ChatGPT, we want to pair user questions with assistant responses
    if (messageData.role === 'user') {
      console.log('[AIV Content] Processing user message');
      ...
      if (isDuplicate) {
        console.log('[AIV Content] Skipping duplicate question');
        return;
      }
      
      // Also check if this exact question was just processed
      if (lastProcessedPairKey === questionKey) {
        console.log('[AIV Content] Skipping recently processed question');
        return;
      }
      ...
      console.log('[AIV Content] Added new question pair:', newPair);
    } else if (messageData.role === 'assistant' && conversationPairs.length > 0) {
      console.log('[AIV Content] Processing assistant message');
      
      // Add assistant message as a response to the last question
      const lastPair = conversationPairs[conversationPairs.length - 1];
      console.log('[AIV Content] Last pair:', lastPair);
      console.log('[AIV Content] New response:', messageData.content);
      ...
      if (lastPair.question === messageData.content) {
        console.log('[AIV Content] ERROR: Same content detected for question and response, skipping');
        return;
      }
      
      // Check conditions for triggering validation
      console.log('[AIV Content] Condition check - lastPair.response:', lastPair.response);
      console.log('[AIV Content] Condition check - messageData.content:', messageData.content);
      console.log('[AIV Content] Condition check - isValidationEnabled:', isValidationEnabled);
      
      // Additional check to ensure we have meaningful content
      const isMeaningful = isMeaningfulResponse(messageData.content);
      console.log('[AIV Content] isMeaningfulResponse result:', isMeaningful);
      
      if (!lastPair.response && messageData.content && isMeaningful) {
        console.log('[AIV Content] Adding response to last pair');
        lastPair.response = messageData.content;
        
        // Trigger validation for this completed pair
        if (isValidationEnabled) {
          console.log('[AIV Content] Triggering validation for completed pair');
          triggerValidationForPair(lastPair);
        } else {
          console.log('[AIV Content] Validation not enabled, skipping validation trigger');
        }
      } else {
        console.log('[AIV Content] Skipping response addition - conditions not met');
        if (lastPair.response) {
          console.log('[AIV Content] Reason: Last pair already has a response');
        }
        if (!messageData.content) {
          console.log('[AIV Content] Reason: New message has no content');
        }
        if (!isMeaningful) {
          console.log('[AIV Content] Reason: Content is not a meaningful response:', messageData.content);
        }
      }
      ...
      if (lastPair.question && lastPair.response) {
        ...
        if (isDuplicate) {
          ...
          console.log('[AIV Content] Removed duplicate conversation pair');
        }
      }
    } else {
      console.log('[AIV Content] Skipping message - not assistant or no conversation pairs');
      if (messageData.role !== 'assistant') {
        console.log('[AIV Content] Reason: Not an assistant message');
      }
      if (conversationPairs.length === 0) {
        console.log('[AIV Content] Reason: No conversation pairs exist');
        
        // If it's an assistant message but we don't have a question yet, try to find the actual question
        const isMeaningful = isMeaningfulResponse(messageData.content);
        console.log('[AIV Content] Standalone isMeaningfulResponse result:', isMeaningful);
        
        if (messageData.role === 'assistant' && messageData.content && isMeaningful) {
          console.log('[AIV Content] Looking for actual user question before creating standalone pair');
          
          // Try to find the actual user question by looking at the DOM
          const userQuestion = findLatestUserQuestion();
          if (userQuestion) {
            console.log('[AIV Content] Found actual user question:', userQuestion);
            ...
            conversationPairs.push(newPair);
            console.log('[AIV Content] Added proper question-response pair:', newPair);
          } else {
            console.log('[AIV Content] Could not find user question, creating standalone pair');
            const newPair = {
            ...
            conversationPairs.push(newPair);
            console.log('[AIV Content] Added standalone assistant pair:', newPair);
          }
        }
      }
    }
    ...
    console.log('[AIV Content] Current conversation pairs:', conversationPairs);
  }

  // Find the latest user question by examining the DOM
  function findLatestUserQuestion() {
    try {
      console.log('[AIV Content] Searching for latest user question in DOM');
      ...
      for (const selector of userMessageSelectors) {
        ...
        console.log(`[AIV Content] Found ${userMessages.length} elements with selector: ${selector}`);
        if (userMessages.length > 0) {
          ...
          if (lastUserMessage && lastUserMessage.textContent) {
            ...
            if (questionText.length > 0) {
              console.log('[AIV Content] Found user question with specific selector:', questionText.substring(0, 50) + '...');
              return questionText;
            }
          }
        }
      ...
      console.log('[AIV Content] Trying broader selectors for user questions');
      ...
      for (const selector of broadSelectors) {
        ...
        console.log(`[AIV Content] Found ${userElements.length} elements with broad selector: ${selector}`);
        if (userElements.length > 0) {
          ...
          if (lastUserElement && lastUserElement.textContent) {
            ...
            if (questionText.length > 0) {
              console.log('[AIV Content] Found user question with broad selector:', questionText.substring(0, 50) + '...');
              return questionText;
            }
          }
        }
      ...
      console.log('[AIV Content] Trying last resort method for user questions');
      const allMessages = document.querySelectorAll('[data-message-author-role]');
      console.log(`[AIV Content] Found ${allMessages.length} total messages with data-message-author-role`);
      
      // Go through messages in reverse order to find the most recent user message
      for (let i = allMessages.length - 1; i >= 0; i--) {
        const message = allMessages[i];
        const role = message.getAttribute('data-message-author-role');
        console.log(`[AIV Content] Message ${i} has role: ${role}`);
        ...
        if (role === 'user') {
          ...
          if (questionText.length > 0) {
            console.log('[AIV Content] Found user question with last resort method:', questionText.substring(0, 50) + '...');
            return questionText;
          }
        }
      }
      
      console.log('[AIV Content] No user question found in DOM');
      return null;
    } catch (error) {
      console.error('[AIV Content] Error finding user question:', error);
      return null;
    }
  }
  ...
  function isMeaningfulResponse(content) {
    if (!content) {
      console.log('[AIV Content] isMeaningfulResponse: Content is null/undefined');
      return false;
    }
    
    // Trim whitespace
    const trimmedContent = content.trim();
    console.log('[AIV Content] isMeaningfulResponse: Trimmed content length:', trimmedContent.length);
    
    // Must have some content
    if (trimmedContent.length < 10) {
      console.log('[AIV Content] isMeaningfulResponse: Content too short');
      return false;
    }
    ...
    for (const pattern of disclaimerPatterns) {
      if (lowerContent.includes(pattern)) {
        ...
        if (trimmedContent.length < pattern.length + 50) {
          console.log('[AIV Content] isMeaningfulResponse: Found disclaimer pattern:', pattern);
          return false;
        }
      }
    }
    ...
    for (const pattern of uiPatterns) {
      if (lowerContent.includes(pattern)) {
        ...
        if (trimmedContent.length < pattern.length + 10) {
          console.log('[AIV Content] isMeaningfulResponse: Found UI pattern:', pattern);
          return false;
        }
      }
    }
    ...
    if (words.length === 1 && uiPatterns.includes(lowerContent)) {
      console.log('[AIV Content] isMeaningfulResponse: Single word UI element detected');
      return false;
    }
    
    console.log('[AIV Content] isMeaningfulResponse: Content is meaningful');
    return true;
  }
  ...
  function isCompleteResponse(content, element) {
    ...
    if (content.length < 15) {
      console.log('[AIV Content] isCompleteResponse: Content too short to be complete');
      return false;
    }
    ...
    if (element) {
      ...
      if (loadingIndicators.length > 0) {
        console.log('[AIV Content] isCompleteResponse: Found loading indicators, response still streaming');
        return false;
      }
      
      // Check if element has the streaming class
      if (element.classList && element.classList.contains('result-streaming')) {
        console.log('[AIV Content] isCompleteResponse: Element has streaming class, response still streaming');
        return false;
      }
      ...
      if (parent) {
        ...
        if (streamingElements.length > 0) {
          console.log('[AIV Content] isCompleteResponse: Found ChatGPT streaming indicators');
          return false;
        }
      }
      ...
      if (cursorElements.length > 0) {
        console.log('[AIV Content] isCompleteResponse: Found cursor elements, response still streaming');
        return false;
      }
    }
    ...
    if (lastChar === '-' || lastChar === ',' || lastChar === ';' || 
      ...
      lastChar === '<' || lastChar === '"' || lastChar === "'") {
    console.log('[AIV Content] isCompleteResponse: Content ends with incomplete punctuation or operator');
    return false;
  }
  ...
  if (trimmedContent.match(/\d\s*[\+\-\*\/]$/) || 
    ...
    console.log('[AIV Content] isCompleteResponse: Content ends with incomplete mathematical expression');
    return false;
  }
  
  // Check for incomplete year ranges (e.g., "1889–1")
  if (trimmedContent.match(/\d{4}[\u2013\u2014]\d?$/)) {
    console.log('[AIV Content] isCompleteResponse: Content ends with incomplete year range');
    return false;
  }
  
  console.log('[AIV Content] isCompleteResponse: Response appears to be complete');
  return true;
}

  // Trigger validation for a completed conversation pair
  function triggerValidationForPair(pair) {
    console.log('[AIV Content] triggerValidationForPair called with pair:', pair);
    ...
    console.log('[AIV Content] Triggering validation for assistant response:', messageData.content.substring(0, 100) + '...');
    console.log('[AIV Content] Full message data:', messageData);
    ...
    chrome.runtime.sendMessage({
      ...
      if (chrome.runtime.lastError) {
        console.error('[AIV Content] Error sending message to background:', chrome.runtime.lastError);
      } else {
        console.log('[AIV Content] Background validation response:', response);
      }
    });
  }
  ...
  function extractMessageData(element) {
    try {
      console.log('[AIV Content] extractMessageData called with element:', element);
      console.log('[AIV Content] Element tag name:', element.tagName);
      console.log('[AIV Content] Element class list:', element.classList);
      console.log('[AIV Content] Element attributes:', element.attributes);
      ...
      const isUserMessage = userSelectors.some(selector => {
        ...
        console.log(`[AIV Content] Checking user selector "${selector}": matches=${matches}, hasParent=${parent !== null}, result=${result}`);
        return result;
      });
      
      if (isUserMessage) {
        role = 'user';
        console.log('[AIV Content] Role identified as user message');
      } else {
        ...
        const isAssistantMessage = assistantSelectors.some(selector => {
          ...
          const parent = element.closest(selector);
          const result = matches || (parent !== null);
          console.log(`[AIV Content] Checking assistant selector "${selector}": matches=${matches}, hasParent=${parent !== null}, result=${result}`);
          return result;
        });
        
        if (isAssistantMessage) {
          role = 'assistant';
          console.log('[AIV Content] Role identified as assistant message');
        } else {
          ...
          if (grandParent && grandParent.hasAttribute('data-message-author-role')) {
            ...
            console.log('[AIV Content] Role from grandparent data-message-author-role:', role);
            ...
            role = 'assistant';
            console.log('[AIV Content] Role defaulted to assistant for text-token-text-primary');
          } else {
            console.log('[AIV Content] Could not determine message role, returning null');
            return null;
          }
        }
      }
      
      console.log('[AIV Content] Determined role:', role);
      ...
      if (element.tagName === 'P') {
        ...
        console.log('[AIV Content] Extracted text from paragraph element, length:', text.length);
        ...
        console.log('[AIV Content] Extracted text from', paragraphs.length, 'paragraphs, total length:', text.length);
      } else if (element.textContent) {
        // Fallback to general text content extraction
        text = element.textContent.trim();
        console.log('[AIV Content] Extracted text from general content, length:', text.length);
      }
      
      console.log('[AIV Content] Extracted text content:', text.substring(0, 100) + (text.length > 100 ? '...' : ''));
      
      // Don't process empty messages
      if (!text) {
        console.log('[AIV Content] No text content found, returning null');
        return null;
      }
      
      // Skip very short messages that are likely UI elements
      if (text.length < 5) {
        console.log('[AIV Content] Text too short, returning null');
        return null;
      }
      ...
      if (text.includes('window.') || text.includes('function(') || text.includes('()=>') ||
        ...
        console.log('[AIV Content] Skipping JavaScript code:', text.substring(0, 50) + '...');
        return null;
      }
      ...
      if (specialCharCount > text.length * 0.3) {
        console.log('[AIV Content] Skipping content with too many special characters:', text.substring(0, 50) + '...');
        return null;
      }
      
      // Skip if it looks like a script tag content
      if (element.tagName === 'SCRIPT' || element.closest('script')) {
        console.log('[AIV Content] Skipping script element content');
        return null;
      }
      ...
      for (const pattern of skipPatterns) {
        if (lowerText.includes(pattern.toLowerCase())) {
          ...
          if (text.length < 30 || 
            ...
            console.log('[AIV Content] Skipping UI element:', text);
            return null;
          }
        }
      }
      ...
      if (element.tagName === 'BUTTON' || element.tagName === 'A' || 
        ...
        console.log('[AIV Content] Skipping button/link element:', text);
        return null;
      }
      ...
      if (element.closest('header') || element.closest('footer') || 
        ...
        console.log('[AIV Content] Skipping header/footer element:', text);
        return null;
      }
      ...
      console.log('[AIV Content] Final extracted message data:', result);
      return result;
    } catch (error) {
      console.error('[AIV Content] Error extracting message data:', error);
      return null;
    }
  }
  ...
  async function triggerManualValidation() {
    console.log('[AIV Content] Triggering manual validation');
        ...
        if (messageData && messageData.role === 'assistant' && messageData.content) {
          ...
          if (messageData.content.length >= 5 && isMeaningfulResponse(messageData.content)) {
            ...
            if (isCompleteResponse(messageData.content, message)) {
              ...
              console.log('[AIV Content] Skipping incomplete assistant message in manual validation');
            }
          }
        }
      });
            ...
            if (messageData && messageData.role === 'assistant' && messageData.content) {
              ...
              if (messageData.content.length >= 5 && isMeaningfulResponse(messageData.content)) {
                ...
                if (isCompleteResponse(messageData.content, message)) {
                  ...
                  console.log('[AIV Content] Skipping incomplete assistant message in manual validation (fallback)');
                }
              }
            }
          }
    ...
    console.log('[AIV Content] Found', assistantMessages.length, 'assistant messages');
    
    if (assistantMessages.length > 0) {
      // Get the last assistant message
      const latestMessage = assistantMessages[assistantMessages.length - 1];
      console.log('[AIV Content] Validating latest assistant message:', latestMessage.data.content.substring(0, 100) + '...');
      ...
      return new Promise((resolve, reject) => {
        chrome.runtime.sendMessage({
          ...
          if (chrome.runtime.lastError) {
            ...
            console.log('[AIV Content] Background response:', response);
            resolve(response);
          }
        });
      });
  ...
  chrome.storage.local.get(['aivValidationEnabled'], function(result) {
    ...
    if (isValidationEnabled) {
      console.log('[AIV Content] AIV Validation was enabled, starting observation');
      startObserving();
    }
  });

  console.log('[AIV Content] AIV Systems Content Script loaded');
} else {
  console.log('[AIV Content] AIV Systems Content Script already loaded, skipping initialization');
}