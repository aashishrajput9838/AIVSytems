...
function loadConversationPairsWithoutValidation() {
  chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
    if (tabs[0]) {
      ...
      chrome.tabs.sendMessage(tabs[0].id, {
        ...
        if (chrome.runtime.lastError) {
          ...
          chrome.runtime.sendMessage({
            ...
            } else if (validationResponse.success && validationResponse.results) {
  ...
  finalDisplayPairs.forEach((pair, index) => {
    ...
    if (validationResults && validationResults.length > 0) {
      ...
      for (let i = 0; i < validationResults.length; i++) {
        ...
        // Check if questions match (another way to match pairs)
        if (normalizedPairQuestion && normalizedResultQuestion && 
            normalizedPairQuestion === normalizedResultQuestion) {
          console.log(`[AIV Popup] Found question match for result ${i}`);
          validationInfo = result;
          break;
        }
        ...
        // For the latest pair, use it even if not exact match (since validation is for latest)
        if (i === validationResults.length - 1 && index === finalDisplayPairs.length - 1) {
          console.log(`[AIV Popup] Using latest validation result for latest pair`);
          validationInfo = result;
          break;
        }
      }
      ...
      if (!isValid && validation.issues && validation.issues.length > 0) {
        pairsHTML += `
          <div class="validation-issues">
            Issues: ${Array.isArray(validation.issues) ? validation.issues.join(', ') : JSON.stringify(validation.issues)}
          </div>
        `;
      } else if (!isValid && validation.issues) {
